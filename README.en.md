...
[![Build](https://img.shields.io/github/actions/workflow/status/Piracola/Floorp_portable/Floorp-Portable-package.yml?style=flat-square&label=Build)](https://github.com/Piracola/Floorp_portable/actions/workflows/Floorp-Portable-package.yml)
[![Release](https://img.shields.io/github/v/release/Piracola/Floorp_portable?style=flat-square&label=Release&color=blue)](https://github.com/Piracola/Floorp_portable/releases/latest)
[![Downloads](https://img.shields.io/github/downloads/Piracola/Floorp_portable/total?style=flat-square&label=Downloads)](https://github.com/Piracola/Floorp_portable/releases)
[![License](https://img.shields.io/github/license/Piracola/Floorp_portable?style=flat-square&label=License)](LICENSE)

[简体中文](README.md) | **English**

A ready-to-run **portable build of Floorp**, rebuilt daily to keep up with official releases.

[Floorp](https://floorp.app/) is a Firefox-based browser developed by a Japanese team, best known for its extensive interface customization and vertical tabs. This project takes the official installer as-is and makes it portable with [libportable](https://github.com/adonais/libportable): all data stays inside the folder you extracted, nothing is written to the registry, nothing pollutes the system, and you can carry the whole thing around on a USB stick.

## Quick start

1. Open the [latest release](https://github.com/Piracola/Floorp_portable/releases/latest).
2. Download `Floorp_<version>.7z`.
3. Extract it anywhere you like, for example `D:\Browser\Floorp`.
...
└── Floorp.lnk                    The shortcut created by running 开始.bat
```
...
2. Rename the old `Floorp` directory to `Floorp_old` as a backup.
3. Extract the `Floorp` directory from the new archive into the same place.
4. Start the browser and confirm that your bookmarks, extensions, and sign-ins are all intact.
5. Once you are satisfied, delete `Floorp_old`.

> Do not delete the whole extracted folder and start over from the new archive; that takes everything in `Profiles/` with it.

## Configuration

The configuration file lives at `Floorp/portable.ini`, and normally you do not need to touch it. The settings people actually use:

| Setting | Default | Description |
|--------|--------|------|
| `Portable` | `1` | Portable mode switch; set it to `0` to fall back to the behavior of a normal installation |
| `PortableDataPath` | `../Profiles` | User data directory, resolved relative to `Floorp/` |
...
Changes take effect only after restarting the browser. See `Floorp/README` for the full list of settings.
...
If either check fails, the build fails and nothing is published. That is what catches injection silently going dead — for example when Floorp rebases onto a new major Firefox release and the portable patching hooks stop taking effect.

The installer itself is also compared byte for byte against the SHA-256 that Floorp publishes with each official release right after download, and any mismatch aborts the build immediately.
...
| File | Purpose |
|------|------|
| `libportable/portable.ini` | The portable patching configuration specific to Floorp |
...
The result is `Floorp_<version>.7z` in the current directory.

## FAQ

### Which file should I download?

`Floorp_<version>.7z`. `Source code` is the repository source and is of no use as a browser.

### Can I keep it on a USB stick?

Yes, that is exactly what a portable build is for. Prefer a simple path such as `U:\Floorp` so you do not run into overly long paths or special characters.
...
First check that `Floorp\floorp.exe` exists. If it does, you can double-click it directly and the browser will start fine; `开始.bat` only exists to generate a convenient shortcut for you.
...
The Floorp installer ships with every language included. Open `Settings → General → Language` in the browser and switch there.
...
| [Floorp](https://github.com/Floorp-Projects/Floorp) | The upstream browser project |
...
The Floorp browser itself remains copyright Floorp Projects and is covered by its own license. The libportable component used for portable patching is covered by its own license and is distributed together with the package.
